...
export const propertyos = {
  ...
  screenFlow: {
    ...
    goal:
      "หน้านี้คือ editor ที่เอเจนต์จะเจอเมื่อเลือก “แก้ต่อ” หลังจบ onboarding — พื้นที่สำหรับปรับเว็บที่ระบบสร้างให้แล้วให้ตรงกับที่ต้องการมากขึ้น ทั้งเลือกหน้าที่จะแก้ เปิด–ปิดและสลับลำดับ section ของหน้านั้น ปรับ layout เนื้อหา และการตั้งค่าของแต่ละส่วน แล้วเห็นผลลัพธ์ทันทีอยู่ในหน้าเดียว โดยทุกหน้าและทุก section ใช้โครงการทำงานชุดเดียวกัน เอเจนต์จึงเรียนรู้ครั้งเดียวแล้วใช้ได้กับทั้งเว็บ",
    /** 3 พาเนลที่ทุกหน้าใน builder ใช้โครงเดียวกัน — ชื่อ Panel N โชว์เป็นแท็กเล็ก ๆ
     *  note = บอกสั้น ๆ ว่าพาเนลนั้นเอาไว้ทำอะไร (บรรทัดเดียวพอ) */
    panels: [
